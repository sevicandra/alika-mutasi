...
export const getAllTarif = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const whereClause = search
      ? {
          [Op.or]: [
            where(col("jenis"), { [Op.like]: `%${search}%` }),
          ],
        }
      : {};
...
export const createTarif = async (req: AuthenticatedRequest, res: Response) => {
  try {
...
export const updateTarif = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  try {
    const { tarif, jenis } = req.body;
    const data = await RefTarif.findByPk(id);
...
export const deleteTarif = async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  try {
    const data = await RefTarif.findByPk(id);
    if (!data) {
